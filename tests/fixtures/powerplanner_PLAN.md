...
### v2.10.0 - R0+R1: SRS + entity dump + capture core (NEXT — awaiting user go)
...
- [ ] R1a [worker: codex SOL]: EntityDump serializer in pure scene layer, cached per scene build; ALL prims covered; Overlay_DumpEntitiesForTest seam; ppops pure tests (round-trip, parent links, screen-rect projection)
...
- [ ] R1c [worker: grok-4.5]: toggle UI polish (app bar Settings group + ribbon button + REC indicator)
...
### v2.10.2 - R3: live-bridge decision (ppsession.exe entities/query CLI vs MCP) — after v2.9.1/2 experience
